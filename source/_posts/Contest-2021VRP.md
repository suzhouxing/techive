...
第一行给出 3 个由空白字符分隔的整数, 分别表示节点数 N, 最大可用车辆数 K, 以及各车辆的载重量上限 Q.
...
注意, 本问题所用算例集中假设行驶时间等于两点间的距离, 目前的距离计算遵循文献与 DIMACS 竞赛中的约定, 使用截断保留 1 位小数的欧氏距离 ($t = \lfloor 10 d \rfloor / 10$), 具体计算方式以 SDK 中的代码为准.
